...
@ApplicationScoped
public class ScoreTreatmentEmphasizeFollowing extends ScoreStrategy {
    ...
    private static final double FRIENDS_WEIGHT;
    ...
    static {
        // Friendship weights: FIRST_HOP=0.5 SECOND_HOP=0.2 CREW_MEMBER=0.5 FRIENDS=0.35
        double firstHop = 0.5;
        double secondHop = 0.2;
        double crewMember = 0.5;
        double friends = 0.35;
        double totalWeight = firstHop + secondHop + crewMember + friends;
        FIRST_HOP_WEIGHT = firstHop / totalWeight;
        SECOND_HOP_WEIGHT = secondHop / totalWeight;
        CREW_MEMBER_WEIGHT = crewMember / totalWeight;
        FRIENDS_WEIGHT = friends / totalWeight;
        // Weights: FRIENDSHIP=0.2 TIME=0.9 ACCOUNT_LIKE=0.5
        double friendship = 0.2;
        double time = 0.9;
    ...
    protected double calculateScore(
        ...
        return ScoreTreatmentEmphasizeFollowingCalculator.calculate(
            video,
            likeCountWeightMap,
            firstHop,
            secondHop,
    ...
    protected static class ScoreTreatmentEmphasizeFollowingCalculator {
        protected static double calculate(
            ...
            double friendshipScore = firstHop * FIRST_HOP_WEIGHT + secondHop * SECOND_HOP_WEIGHT
                + crewMember * CREW_MEMBER_WEIGHT + friends * FRIENDS_WEIGHT;
