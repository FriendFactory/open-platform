...
module "video-gen" {
  ...
  service_container_image_url                = "304552489232.dkr.ecr.eu-central-1.amazonaws.com/video-gen"
...
resource "aws_iam_role_policy" "video-gen-task-execution-role-policy" {
  name = "video-gen-task-execution-role-policy"
  role = module.video-gen.service-iam-role.id
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Sid      = "UseLogGroups"
        Effect   = "Allow"
        Resource = [module.video-gen.service-logs.arn, data.aws_cloudwatch_log_group.session-manager-log-group.arn]
      },
      {
        Action = [
          "kms:Decrypt",
