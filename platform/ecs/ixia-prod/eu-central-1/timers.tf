...
module "timers" {
  depends_on                                 = [module.ixia-prod-ecs-cluster]
  source                                     = "../../../tf-modules/ecs-service"
  vpc_name                                   = "ixia-prod"
  region                                     = "eu-central-1"
  ecs_cluster_name                           = "ixia-prod"
  service_name                               = "timers"
  lb_names                                   = [module.ixia-prod-ecs-cluster.ecs-alb.name]
  lb_sg_name                                 = module.ixia-prod-ecs-cluster.ecs-alb-sg.name
  lb_listener_priority                       = 10
  service_host_header                        = { (module.ixia-prod-ecs-cluster.ecs-alb.name) = module.ixia-prod-ecs-cluster.ecs-alb-root-url }
  service_health_check_path                  = "hello"
  service_container_port                     = 8080
  service_container_image_url                = "${data.aws_ecr_repository.timers.repository_url}:ixia-prod"
  service_subnet_ids                         = module.ixia-prod-ecs-cluster.ecs-subnet-ids
  ...
  service_extra_sgs                          = [data.aws_security_group.ixia-prod-db-client.id]
  cpu_architecture                           = "ARM64"
  target_group_name                          = "ixia-prod-timers-tg"
  service_role_name                          = "ixia-prod-timers-task-execution-role"
  cloudwatch_log_group_name                  = "ixia-prod-timers-service-logs"
}

data "aws_caller_identity" "current" {}

data "aws_vpc" "ixia-prod" {
  tags = {
    Name = "ixia-prod"
  }
}

data "aws_security_group" "ixia-prod-db-client" {
  name   = "ixia-prod-rds-client-sg"
  vpc_id = data.aws_vpc.ixia-prod.id
}

resource "aws_iam_role_policy" "timers-task-execution-role-policy" {
  name = "ixia-prod-timers-task-execution-role-policy"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Sid      = "UseLogGroups"
        Effect   = "Allow"
        Resource = [module.timers.service-logs.arn, module.ixia-prod-ecs-cluster.session_manager_cloudwatch_log_group.arn]
      },
      {
        Action = [
          "kms:Decrypt",
        ...
        Sid      = "UseKmsKey"
        Effect   = "Allow"
        Resource = module.ixia-prod-ecs-cluster.session-manager-kms-key.arn
      },
      {
      ...
      },
      {
        ...
        Effect   = "Allow"
        Resource = ["arn:aws:ssm:*:${data.aws_caller_identity.current.account_id}:parameter/timers/ixia-prod/*"]
      },
      {
        Action = [
          "sqs:ReceiveMessage",
...
resource "aws_sqs_queue" "timers-input-queue" {
  ...
  name                      = "ixia-prod-timers-input-queue"
}

resource "aws_sqs_queue" "timers-input-queue-deadletter" {
  // 14 days
  message_retention_seconds = 1209600
  name                      = "ixia-prod-timers-input-queue-deadletter"
...
resource "aws_sns_topic" "video-template-mapping" {
  name = "ixia-prod-video-template-mapping"
}
...
resource "aws_sns_topic" "template-updated" {
  name = "ixia-prod-template-updated"
}
...
resource "aws_sns_topic" "group-unfollowed" {
  name = "ixia-prod-group-unfollowed"
}
